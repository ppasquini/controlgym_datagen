...
class KuramotoSivashinskyEnv(PDE):
    ...
    def select_init_state(self, init_amplitude=0.5):
        ...
        init_state = (
            init_amplitude
            * np.cos(5 * np.pi * self.domain_coordinates / self.domain_length)
            #* np.sin(12 * np.pi * self.domain_coordinates / self.domain_length)**6
        )
        return init_state
    ...
    def _compute_fourier_linear_op(self):
        ...
        """
        fourier_linear_op = self.domain_wavenumbers**2 - self.viscosity*self.domain_wavenumbers**4
